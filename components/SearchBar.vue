<script setup lang="ts">
const props = defineProps<{
    inputClasses: string[],
    modelValue: string,
    placeholder: string
}>()

const inputClassesProperties = computed(() => props.inputClasses.join(' '))
</script>

<template>
    <div class="flex bg-gray items-center">
        <div class="text-gray-dark text-sm px-3 flex gap-4 items-center">
            All <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M4.24106 7.7459C4.53326 7.44784 4.99051 7.42074 5.31272 7.66461L5.40503 7.7459L12 14.4734L18.595 7.7459C18.8872 7.44784 19.3444 7.42074 19.6666 7.66461L19.7589 7.7459C20.0511 8.04396 20.0777 8.51037 19.8386 8.83904L19.7589 8.93321L12.582 16.2541C12.2898 16.5522 11.8325 16.5793 11.5103 16.3354L11.418 16.2541L4.24106 8.93321C3.91965 8.60534 3.91965 8.07376 4.24106 7.7459Z"
                    fill="#999999" />
            </svg>

        </div>
        <input type="search" :class="inputClassesProperties" :placeholder="placeholder"
            @input="$emit('update:modelValue', $event.target && ($event.target as HTMLInputElement).value)"
            class="h-full" />
    </div>
</template>